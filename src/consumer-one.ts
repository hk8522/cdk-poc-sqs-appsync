...
export async function handler(event: SQSEvent) {
  ...
  for (const item of event.Records) {
    ...
    const resp = await makeRequest(auction.id, body.bid);
...
const makeRequest = (auctionId: string, bid: number) => {
  const bidCompleteMutation = `mutation bidComplete($auctionId: ID!, $bid: Int!) {
    bidComplete(auctionId: $auctionId, bid: $bid) {
        auctionId
        bid
      }
    }`;
